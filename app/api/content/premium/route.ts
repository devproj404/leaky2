...
export async function GET(request: Request) {
  try {
    ...
    // Create cache key for premium content
    const cacheKey = `content:premium:page:${page}:limit:${limit}:sort:${sort}`

    // Use cache for premium content
    const data = await getCached(
      cacheKey,
      async () => {
        // Fetching premium content from database using admin client
        ...
        if (error) {
          // Database error fetching premium content
    ...
    console.error('Error in premium content API:', error)
    return NextResponse.json(
      { 
